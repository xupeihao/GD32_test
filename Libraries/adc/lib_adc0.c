...
#define ADCx_RESOLUTION     LIB_ADC0_RESOLUTION
#define ADCx_DATAALIGN      LIB_ADC0_DATAALIGN

#define ADCx_CHN_TYPE       LIB_ADC0_CHN_TYPE
#define ADCx_CHNS_LEN       LIB_ADC0_CHNS_LEN
#define ADCx_CHNS           LIB_ADC0_CHNS

#define ADCx_SAMPLETIMES    LIB_ADC0_SAMPLETIMES
...
static void ADCx_DMA_config() {
    ...
    uint32_t dmax_src_len = ADCx_CHNS_LEN;
...
void lib_adc0_init() {
    ...
    for(uint8_t i = 0; i < adcx_chns_len; i++) {
        adc_routine_channel_config(ADCx, i, adcx_chns[i], adcx_samples[i]);
    ...
    /////////// 模式配置
    adc_special_function_config(ADCx, ADC_SCAN_MODE, ENABLE);// 取消扫描模式
    adc_special_function_config(ADCx, ADC_CONTINUOUS_MODE, ENABLE);//取消连续模式
...
#else
    adc_special_function_config(ADCx, ADC_INSERTED_CHANNEL_AUTO, ENABLE);//取消连续模式
#endif
