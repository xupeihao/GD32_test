...
void lib_i2c2_init() {
    ...
    rcu_periph_clock_enable(I2C2_SCL_PORT_RCU);
		// 设置复用功能
    gpio_af_set(I2C2_SCL_PORT, I2C2_SDA_AF, I2C2_SCL_PIN);
		// 设置输出模式
    gpio_mode_set(I2C2_SCL_PORT, GPIO_MODE_AF, GPIO_PUPD_NONE, I2C2_SCL_PIN);
    gpio_output_options_set(I2C2_SCL_PORT, GPIO_OTYPE_OD, GPIO_OSPEED_MAX, I2C2_SCL_PIN);
    
    // 时钟配置
    rcu_periph_clock_enable(I2C2_SDA_PORT_RCU);
    // 设置复用功能
    gpio_af_set(I2C2_SDA_PORT, I2C2_SDA_AF, I2C2_SDA_PIN);
		// 设置输出模式
    gpio_mode_set(I2C2_SDA_PORT, GPIO_MODE_AF, GPIO_PUPD_NONE, I2C2_SDA_PIN);
...
uint8_t lib_i2c2_write(uint8_t addr, uint8_t reg, uint8_t* data, uint32_t data_len) {
